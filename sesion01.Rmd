...
6. Usa el botón $\includegraphics[width=0.7cm]{fig/01-fig20-boton_fork_GitHub.png}$ que aparece arriba a la derecha para generar tu copia de ese repositorio. Cuando el fork acabe (asegúrate de que es tu nombre de usuario el que aparece en la barra de direcciones del navegador). Usa el botón verde 
...
7. En RStudio usa el menú $\text{\menu[,]{File, New Project}}$ y en la ventana que aparece selecciona *Version Control*. A continuación selecciona *Git* e introduce en el primer campo la URL que has copiado en GitHub en el paso anterior. Acepta *fmad2122* como nombre del repositorio y, **muy importante**, usa el botón *browse* para seleccionar la ubicación del clon local del repositorio de la asignatura. Deberías situarlo dentro de tu carpeta de la asignatura la que hemos llamado FMAD [\textcolor{blue}{en esta figura}](estructura_carpetas)
