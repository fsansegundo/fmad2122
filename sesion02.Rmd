...
+ Las frecuencias relativas suman siempre 1. Compruébalo:
  ```{r echo=-1, results= 'hide'}
...
+ Las *frecuencias acumuladas* se usan con variables discretas para responder a la pregunta *"¿cuántos valores hay que sean menores o iguales que ...?"* En R se obtienen con:\small
  ...
  ```
  \normalsize que nos dice, por ejemplo, que en la tabla hay `r cumsum(table(mpg$cty))[7]` valores menores o iguales que `r (sort(unique(mpg$cty)))[7]`.  **Ejercicio:** (1) usa `mutate` con una función adecuada para hacer esto en `dplyr`. (2) ¿qué pregunta responden las frecuencias acumuladas relativas? ¿son distintas de las *relativas acumuladas*?
...
+ Dibujar un boxplot con R básico es muy sencillo. Por ejemplo, para la variable `cty`:
  ```{r echo=-1, comment=NULL, fig.align='center', out.width = "50%"}
...
+ En este caso usamos:
  ```{r echo=-1, comment=NULL, fig.align='center', out.width = "60%"}
